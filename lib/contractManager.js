...
const wrapFunction = (formatter, solidityFunction) => {
  const wrapMethod = (name) =>
    function (...args) {
      if ((args.length > 0) && (type.is(Function, I.get(-1, args)))) {
        const callback = I.get(-1, args)
        ...
        return solidityFunction[name](...args.concat(R.identity))
      }
    }

  const wrapped = I.toObject(I.map(
