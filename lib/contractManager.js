...
const erisToWeb3Address = (address) =>
  '0x' + address

const erisToWeb3 = (args) =>
  I.map((arg) => utils.isAddress(arg) ? erisToWeb3Address(arg) : arg, args)

const web3ToErisAddress = (address) =>
  address.slice(2)

const web3ToEris = (result) =>
  utils.isStrictAddress(result) ? web3ToErisAddress(result) : result
...
const wrapFunction = (formatter, solidityFunction) => {
  const wrapMethod = (name) =>
    function (...args) {
      ...
      if ((web3Args.length > 0) && (type.is(Function, I.get(-1, web3Args)))) {
