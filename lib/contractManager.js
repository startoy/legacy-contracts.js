...
const wrapFunction = (formatter, solidityFunction) => {
  const call = solidityFunction.call
  const sendTransaction = solidityFunction.sendTransaction

  return Object.assign(solidityFunction, {
    call () {
      const callback = I.get(-1, arguments)

      const callbackWrapper = (error, result) =>
        error ? callback(error) : callback(null, formatter(result))

      return call.apply(
        this,
        I.concat(I.slice(0, -1, arguments), callbackWrapper)
      )
    },
