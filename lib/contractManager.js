...
web3ToEris.method([/^0x[0-9a-fA-F]*$/],
  I.slice(2, Infinity)
)

web3ToEris.method([is(Array)],
  I.map(web3ToEris)
)

const wrapPayload = (payload) =>
  R.assoc('data', web3ToEris(payload.data), payload)
    ...
    function (...args) {
      ...
      if ((web3Args.length > 0) && (is(Function, I.get(-1, web3Args)))) {
        ...
        const callbackWrapper = (error, result) =>
          error
            ? callback(error)
            : callback(null, formatter(web3ToEris(solidityFunction._constant
              ? result
